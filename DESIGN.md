# nav导航页（我的收藏夹）设计与实现说明

> 项目英文名：`Nav-Du`。用于部署一个个人常用网站导航页，支持强检索（含拼音/拼音缩写）、深浅色主题、favicon 防盗链/跨域兜底，并提供简单的管理后台用于维护链接。
...
- 管理后台（默认 `/admin`，可配置）：登录后支持添加/编辑/导入/导出/重置
...
登录防暴力（可选，默认开启）：
- `LOGIN_RATE_LIMIT_WINDOW_SECONDS`（可选）：统计窗口（秒），默认 60
- `LOGIN_RATE_LIMIT_MAX_FAILS`（可选）：窗口内最大失败次数，默认 8
- `LOGIN_RATE_LIMIT_LOCK_SECONDS`（可选）：触发后锁定时间（秒），默认 300

KV：
- `NAV_CONFIG_KV`：存储云端配置（同时用于登录防暴力的计数存储）
